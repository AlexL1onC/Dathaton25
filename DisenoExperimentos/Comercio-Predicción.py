def probabilidad_consumo_en_dia(df, dia_objetivo, usuario):
    ...
    # Paso 3: predecir el comercio más probable en el día siguiente
    dia_siguiente = dia_objetivo + 1
...
resultado = probabilidad_consumo_en_dia(data, dia_objetivo=15, usuario='Usuario 1')
print(f"Comercio top del usuario: {resultado['comercio_top_usuario']}")
print(f"Probabilidad de consumo el día 15: {resultado['probabilidad_consumo_en_dia']}%")
print(f"Predicción para el día {resultado['prediccion_dia_siguiente']['dia']}: {resultado['prediccion_dia_siguiente']['comercio_mas_probable']}")
